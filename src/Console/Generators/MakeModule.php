...
class MakeModule extends Command
{
     ...
     * @var string
     */
    protected $description = 'Edutalk modules generator.';
    ...
    protected function step1()
    {
        ...
        $this->container['name'] = $this->ask('Name of module:', 'Edutalk ' . $this->container['alias']);
        ...
        $this->container['namespace'] = $this->ask('Namespace of module:', 'Edutalk\\' . $this->acceptedTypes[$this->moduleType] . '\\' . studly_case($this->container['alias']));
    ...
    protected function makeModuleFolder()
    {
        switch ($this->moduleType) {
            case 'base':
                if (!$this->files->isDirectory(edutalk_base_path())) {
                    $this->files->makeDirectory(edutalk_base_path());
                }
                return 'edutalk_base_path';
                break;
            case 'plugin':
            default:
                if (!$this->files->isDirectory(edutalk_plugins_path())) {
                    $this->files->makeDirectory(edutalk_plugins_path());
                }
                return 'edutalk_plugins_path';
                break;
        }
    }
